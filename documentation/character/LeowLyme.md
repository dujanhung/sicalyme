<h2>
LeowLyme
</h2>

<details>
<summary>
🏷️
</summary>
<table><tr><td>
<h2>
🏷️ metadata
</h2>
<table><tr><td>

<details>
<summary>
🖼️
</summary>
<table><tr><td>
<h2>
🖼️ icon
</h2>
<table><tr><td>
<img src="https://github.com/dujanhung/sicalyme/blob/main/resource/svg/character/LeowLyme.svg">
</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
🔑
</summary>
<table><tr><td>
<h2>
🔑 quote
</h2>
<table><tr><td>
<h3>
a typo that EOW without M
</h3>
</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
🤔
</summary>
<table><tr><td>
<h2>
🤔 about
</h2>
<table><tr><td>
a
</td></tr></table>
</td></tr></table>
</details>

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
🔍
</summary>
<table><tr><td>
<h2>
🔍 general information
</h2>
<table><tr><td>
<table><tr><td>
<h2>
🏷️ full name
</h2>
<table><tr><td>
Leo LeowLyme
</td></tr></table>
</td></tr></table>
<table><tr><td>
<h2>
🔑 alias
</h2>
<table><tr><td>
Leow
</td></tr></table>
</td></tr></table>
<table><tr><td>
<h2>
🚻 gender
</h2>
<table><tr><td>
♂️ male
</td></tr></table>
</td></tr></table>
<table><tr><td>
<h2>
🍽️ diet
</h2>
<table><tr><td>
🍎 fruit
</td></tr></table>
<table><tr><td>
🥦 veggie
</td></tr></table>
<table><tr><td>
🍗 meat
</td></tr></table>
<table><tr><td>
🍚 flour
</td></tr></table>
<table><tr><td>
⚡ electricity.
</td></tr></table>
</td></tr></table>
<table><tr><td>
<h2>
😋 favorite food
</h2>
<table><tr><td>
🥜 peanut
</td></tr></table>
</td></tr></table>
<table><tr><td>
<h2>
😃 favorite toy
</h2>
<table><tr><td>
📐 quantum delta
<sup>
[undocumented]
</sup>
</td></tr></table>
</td></tr></table>
</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
👀
</summary>
<table><tr><td>
<h2>
👀 appearance
</h2>
<table><tr><td>

* he has a head which is attached on a motorized shaft.
* he has 2 parallel tracks on each side, each track has 4 wheels.
* he has a cannon in front of his face. 
* he has 2 square eyes on the sides of his head.
* he has 2 triangle ears on top of his head, each one face opposite to each other.
* he has a bracket from back of his head.
* he has a tail with 4 bones, each bones are attached with universal joints. the tail-tip is cropped-shape.

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
⚙️
</summary>
<table><tr><td>
<h2>
<img src="https://github.com/dujanhung/sicalyme/blob/main/resource/svg/icon/setting.svg" width="100" height="100"> functionality
</h2>
<table><tr><td>

* he don't need a driver, because he is controlled by an offline AI system.
* he can communicate to human. although he can't speak, he can use other ways to communicate.
* his neck can twist around freely, with motor for source force, and absorber for stability.
* his cannon can bend up and down, to be able to arm at airborne targets.
* he can jump, using pistons inside the tank tracks as source force, and absorber for movement stability.
* his tank tracks can move forward and backward as both tracks go to the same direction; it can turn left and right as each tracks go opposite to each other; however, it can't strafe left and right, unless it's replaced with external steering components.
* his cannon can shoot bullets. it's not only a long-range weapon, but also a combat weapon when used in a short distance.
* his eye can see colors as well as human's eye. however, he cannot see a same object with both eyes.
* his hearing ability can detect the low and high frequency sounds.
* his bracket is mainly used as a passenger seat for a human. also, it can store other items, and liquid. however, long-term liquid contact is harmful, as it eat though the bracket over time.
* his tail is mainly used for balance. also, it can grasp items.

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
🗓️
</summary>
<table><tr><td>
<h2>
🗓️ history
</h2>
<table><tr><td>

* his color used to be purple and blue. however, due to swimming in the lime ocean for too long and often, his skin react with those salty water, which leads to it's color changed to lime and brown.
* his first name is <b>MeowLime</b>

</td></tr></table>
</td></tr></table>
</details>


<details>
<summary>
🛠️
</summary>
<table><tr><td>
<h2>
🛠️ physic
</h2>
<table><tr><td>

* ⚖️<b>mass</b>: 20 kg
* 📏<b>height</b>: 3.125 m
* 🛣️<b>movement speed</b>: 50 km/h
* 💪<b>push/pull strength</b>: 100 N
* ⚡<b>battery voltage</b>: 25 V

* he cannot swim or fly.
* he can move on ground, and jump.

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
🧬
</summary>
<table><tr><td>
<h2>
🧬 biology
</h2>
<table><tr><td>

* 🌬️<b>lung volume</b>: 2 m<sup>3</sup>
* 💧<b>urine volume</b>: 200 mL
* 💉<b>blood volume</b>: 7 L
* 🥁<b>heart rate</b>: 75 BPM
* 🧯<b>blood pressure</b>: minimum 70 mmHg, maximum 120 mmHg

* he can eat food, and due to eating foods, LeowLyme can be able to release waste as will.

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
🔌
</summary>
<table><tr><td>
<h2>
🔌 application
</h2>
<table><tr><td>

* secure the LIME castle within airborne.

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
📎
</summary>
<table><tr><td>
<h2>
📎 gallery
</h2>
<table><tr><td>

<a href="https://dash.makersempire.com/designs/753f10c2-d838-4150-9b02-c0d325b6f431?uid=true&download=true&token=de317cb3fd0b3fa15831e007dbe7e847">
LeowLyme 3D preview in MakersEmpire
</a>

</td></tr></table>
</td></tr></table>
</details>

<details>
<summary>
💡
</summary>
<table><tr><td>
<h2>
💡 reference
</h2>
<table><tr><td>

* The_LeowLyme

</td></tr></table>
</td></tr></table>
</details>

<h2>
 
</h2>

<details>
<summary>
🛣️
</summary>
<table><tr><td>
<h2>
🛣️ navigation
</h2>
<table><tr><td>

* a

</td></tr></table>
</td></tr></table>
</details>
